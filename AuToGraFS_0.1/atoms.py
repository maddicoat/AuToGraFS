...
class Atoms(object):
    ...
    def __init__(self, symbols=None,
        ...
        if hasattr(symbols, 'GetUnitCell'):
            ...
            data = [[atom.get_raw(name) for atom in symbols]
                    ...
                    ['position', 'number', 'tag', 'momentum',
                     'mass', 'magmom', 'charge',
                     'original_index', 'current_index', 'fragmentID',
                     'mmtype', 'bondlist']]
        ...
        if atoms is not None:
            ...
            if fragmentIDs is None and atoms.has('fragmentIDs'):
                fragmentIDs = atoms.get_fragmentIDs()
        ...
        self.set_fragmentIDs(default(fragmentIDs, 0))
        self.set_mmtypes(default(mmtypes, 0))
    ...
    def set_mmtypes(self, mmtypes):
        ...
        self.set_array('mmtypes', mmtypes, object, ())
    ...
    def get_mmtypes(self):
        ...
        if 'mmtypes' in self.arrays:
            ...
            return np.empty(len(self), object)
...
#        return cod
    ...
    def _get_mmtypes(self):
        """Return reference to mmtypes array for in-place manipulations."""
        return self.arrays['mmtypes']


    mmtypes = property(_get_mmtypes, set_mmtypes,
                         doc='Attribute for direct ' +
                         'manipulation of the mmtypes.')
...
#MAA additions here to deal with bondlists
    def update_bondlists_add(self):
        ...
        for a in self: #If we're adding two things and only one of them has bondlists, then add an empty bondlist for things to work appropriately
            if not a.bondlist:
                a.bondlist = {}
            #    print a, "|", 'has no bondlist'
            #else:
            #    print a, "|", a.bondlist
        ...
        if self.has('bondlists'):
            ...
            for a in self:
                if a.bondlist:
                    if a.index + 1 != a.current_index:
                        #print "UBA", a.original_index,a.current_index, a.index + 1
                        if a.current_index not in atom_mapping:
                            atom_mapping[a.current_index]=a.index + 1 #Mapping must be unique, is this assured?
                        else:
    ...
    #Popping is a sufficiently different operation to adding, so we need different coding
    ...
    def add_group(self, group, ind_self, ind_group):
        ...
        print "Entering add_group: ",ind_self, ind_group 
        print group
        #First check that each bonding atom only has one connection
        print self[ind_self].bondlist
        ...
        for key in self[ind_self].bondlist.keys():
            ...
            print "Conn_self=",conn_self
        for key in group[ind_group].bondlist.keys():
            conn_group=key-1 
            print "Conn_group=",conn_group
        ...
        #atoms = self.copy()
        self += group # current indices are updated
        print "New len,", len(self)
        #Form bond between the two conn atoms
        conn_group+=index_offset 
        ind_group+=index_offset 
        print "CG,IG", conn_group,ind_group
        self.write('ag_early_temp.xyz')
        self[conn_self].bondlist[conn_group+1]=1    
        self[conn_group].bondlist[conn_self+1]=1    
        print "bonding ",conn_self, " to ", conn_group+1
        print "bonding ",conn_group, " to ", conn_self+1
        print self[conn_self].bondlist
        print self[conn_group].bondlist
        del self[conn_self].bondlist[ind_self+1]
        del self[conn_group].bondlist[ind_group+1]
        #Delete the two index self
        self.pop(ind_group) #Larger by construction
        self.pop(ind_self)
        #self.write('ag_temp.inp')
        self.write('ag_temp.xyz')
        #return atoms

    def get_atom_index(self,frag_id,orig_index):
        """return the index of the atom with fragment_ID=frag_id and original_index=orig_index."""
        fragIDs = self.get_fragmentIDs()
        orig_indices = self.get_original_indices()
        this_index = [item for item in range(len(fragIDs)) if fragIDs[item] == frag_id and orig_indices[item] == orig_index]
        if len(this_index) != 1:
            raise RuntimeError('More than one atom has the supplied fragmentID and original_index!')
        return this_index[0]
