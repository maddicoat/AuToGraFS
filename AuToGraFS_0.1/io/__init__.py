...
def read(filename, index=-1, format=None):
    ...
    deMon Nano UFF inp         uff
    model molecule (topology)  model
    Gaussian input             com
    ADF input                  run
    GULP input                 gin
    =========================  ===========
    ...
    if format == 'uff':
        from ase.io.uff import read_uff
        return read_uff(filename, index)

    if format == 'model':
        from ase.io.model import read_model
        return read_model(filename, index)

    if format == 'com':
        from ase.io.g09 import read_com
        return read_com(filename)

    if format == 'gin':
        from ase.io.gin import read_gin
        return read_gin(filename)

    if format == 'run':
        from ase.io.adf import read_adf
        return read_adf(filename)
...
##                       }.get(suffix, suffix)
    ...
    if format == 'inp':
        format= 'uff'
        from ase.io.uff import write_uff
        write_uff(filename, images)
        return
    if format == 'gin':
        from ase.io.gin import write_gin
        write_gin(filename, images)
        return
    if format == 'run':
        from ase.io.adf import write_adf
        write_adf(filename, images)
        return
    elif format == 'in':
        format = 'aims'
    elif format == 'tmol':
...
def filetype(filename):
    ...
    if filename.lower().endswith('.run'):
        return 'run'
