...
def make_pto(names,sizes):

    #work out the expansion factor
    factor = sum(sizes)  * 0.90 #shrink it a little bit

    a = 2.8284 * factor
    
    # C -> triangles, N => squares
    pto = crystal(['C', 'N'], [(0.25, 0.25, 0.25), (0.25,0.0,0.5)], spacegroup=223, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(pto),len(pto)) )
    pbc_bond_matrix = np.zeros( (len(pto),len(pto)) )
    ...
    for atom in pto:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, pto[index].symbol, offset, pto.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in pto:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([pto[index].symbol for index in indices])
            symbol_string = ''.join(sorted([pto[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += pto[index]
                    model_molecule[n_obj].positions[-1] = pto.positions[index] + np.dot(offset, pto.get_cell())
    ...
    for atom in pto:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, pto[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
