...
def make_srs_c4(names,sizes):
    ...
    srs_c4 = crystal(['C', 'N'], [(0.25, 0.25, 0.25), (0.25, 0.0, 0.5)], spacegroup=208, #P4(2)32
                   cellpar=[a, a, a, 90, 90, 90])
    
    
    #write('test.xyz',srs_c4)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(srs_c4),len(srs_c4)) )
    pbc_bond_matrix = np.zeros( (len(srs_c4),len(srs_c4)) )
    ...
    for atom in srs_c4:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, srs_c4[index].symbol, offset, srs_c4.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in srs_c4:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([srs_c4[index].symbol for index in indices])
            symbol_string = ''.join(sorted([srs_c4[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if srs_c4[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += srs_c4[index]
                        model_molecule[n_obj].positions[-1] = srs_c4.positions[index] + np.dot(offset, srs_c4.get_cell())
    ...
    for atom in srs_c4:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, srs_c4[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
