...
def make_srs_a_c(names,sizes):
    ...
    srs_a_c = crystal(['C', 'N' , 'N'], [(0.125, 0.058, 0.192), (0.0915, 0.125, 0.1585), (0.125, 0.0, 0.25)], spacegroup=230, #Ia-3d
                   cellpar=[a, a, a, 90, 90, 90])
    
    
    #write('test.xyz',srs_a_c)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(srs_a_c),len(srs_a_c)) )
    pbc_bond_matrix = np.zeros( (len(srs_a_c),len(srs_a_c)) )
    ...
    for atom in srs_a_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, srs_a_c[index].symbol, offset, srs_a_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in srs_a_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([srs_a_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([srs_a_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if srs_a_c[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += srs_a_c[index]
                        model_molecule[n_obj].positions[-1] = srs_a_c.positions[index] + np.dot(offset, srs_a_c.get_cell())
    ...
    for atom in srs_a_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, srs_a_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
    ...
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_a_c.get_cell()[0][0],
               srs_a_c.get_cell()[0][1],
               srs_a_c.get_cell()[0][2]))
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_a_c.get_cell()[1][0],
               srs_a_c.get_cell()[1][1],
               srs_a_c.get_cell()[1][2]))
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_a_c.get_cell()[2][0],
               srs_a_c.get_cell()[2][1],
               srs_a_c.get_cell()[2][2])) 
