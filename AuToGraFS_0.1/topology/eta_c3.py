...
def make_eta_c3(names,sizes):
    ...
    eta_c3 = crystal(['C', 'N' , 'N'], [(0.2207, 0.0, 0.0), (0.5, 0.0, 0.0), (0.1104, 0.0, 0.5)], spacegroup=180, #P6(2)22
                   cellpar=[a, a, b, 90, 90, 120])
    
    
    #write('test.xyz',eta_c3)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(eta_c3),len(eta_c3)) )
    pbc_bond_matrix = np.zeros( (len(eta_c3),len(eta_c3)) )
    ...
    for atom in eta_c3:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, eta_c3[index].symbol, offset, eta_c3.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in eta_c3:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([eta_c3[index].symbol for index in indices])
            symbol_string = ''.join(sorted([eta_c3[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if eta_c3[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += eta_c3[index]
                        model_molecule[n_obj].positions[-1] = eta_c3.positions[index] + np.dot(offset, eta_c3.get_cell())
    ...
    for atom in eta_c3:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, eta_c3[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
