...
def make_pth(names,sizes):
    ...
    pth = crystal(['C', 'N'], [(0.0, 0.0, 0.5), (0.5,0.0,0.0)], spacegroup=180, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(pth),len(pth)) )
    pbc_bond_matrix = np.zeros( (len(pth),len(pth)) )
    ...
    for atom in pth:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, pth[index].symbol, offset, pth.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in pth:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([pth[index].symbol for index in indices])
            symbol_string = ''.join(sorted([pth[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += pth[index]
                    model_molecule[n_obj].positions[-1] = pth.positions[index] + np.dot(offset, pth.get_cell())
    ...
    for atom in pth:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, pth[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
