...
def make_gar(names,sizes):
    ...
    gar = crystal(['C', 'N'], [(0.375, 0.0, 0.25), (0.0,0.0,0.0)], spacegroup=230, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(gar),len(gar)) )
    pbc_bond_matrix = np.zeros( (len(gar),len(gar)) )
    ...
    for atom in gar:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, gar[index].symbol, offset, gar.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in gar:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([gar[index].symbol for index in indices])
            symbol_string = ''.join(sorted([gar[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += gar[index]
                    model_molecule[n_obj].positions[-1] = gar.positions[index] + np.dot(offset, gar.get_cell())
    ...
    for atom in gar:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, gar[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
