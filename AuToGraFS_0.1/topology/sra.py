...
def make_sra(names,sizes):
    ...
    sra = crystal(['C', 'N', 'N', 'N'], [(0.15340, 0.25000, 0.10240), (0.5, 0.75, 0.602400), (0.25, 0.25, 0.25), (0.346600, 0.5, 0.5)], spacegroup=74, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(sra),len(sra)) )
    pbc_bond_matrix = np.zeros( (len(sra),len(sra)) )
    ...
    for atom in sra:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, sra[index].symbol, offset, sra.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in sra:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([sra[index].symbol for index in indices])
            symbol_string = ''.join(sorted([sra[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += sra[index]
                    model_molecule[n_obj].positions[-1] = sra.positions[index] + np.dot(offset, sra.get_cell())
    ...
    for atom in sra:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, sra[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
