...
def make_srs_c4s(names,sizes):
    ...
    srs_c4s = crystal(['C', 'N', 'N'], [(0.0, 0.25, 0.0), (0.25, 0.75, 0.0), (0.375, 0.375, 0.25)], spacegroup=133, setting=2, #P4(2)nbc
                   cellpar=[a, a, b, 90, 90, 90])
    
    
    #write('test.xyz',srs_c4s)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(srs_c4s),len(srs_c4s)) )
    pbc_bond_matrix = np.zeros( (len(srs_c4s),len(srs_c4s)) )
    ...
    for atom in srs_c4s:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, srs_c4s[index].symbol, offset, srs_c4s.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in srs_c4s:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([srs_c4s[index].symbol for index in indices])
            symbol_string = ''.join(sorted([srs_c4s[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if srs_c4s[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += srs_c4s[index]
                        model_molecule[n_obj].positions[-1] = srs_c4s.positions[index] + np.dot(offset, srs_c4s.get_cell())
    ...
    for atom in srs_c4s:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, srs_c4s[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
    ...
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_c4s.get_cell()[0][0],
               srs_c4s.get_cell()[0][1],
               srs_c4s.get_cell()[0][2]))
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_c4s.get_cell()[1][0],
               srs_c4s.get_cell()[1][1],
               srs_c4s.get_cell()[1][2]))
    f.write('%8.3f %8.3f %8.3f \n' %
              (srs_c4s.get_cell()[2][0],
               srs_c4s.get_cell()[2][1],
               srs_c4s.get_cell()[2][2])) 
