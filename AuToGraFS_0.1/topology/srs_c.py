...
def make_srs_c(names,sizes):
    ...
    srs_c = crystal(['C', 'N'], [(0.125, 0.125, 0.125), (0.0, 0.25, 0.125)], spacegroup=230, #Ia-3d
                   cellpar=[a, a, a, 90, 90, 90])
    
    
    #write('test.xyz',srs_c)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(srs_c),len(srs_c)) )
    pbc_bond_matrix = np.zeros( (len(srs_c),len(srs_c)) )
    ...
    for atom in srs_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, srs_c[index].symbol, offset, srs_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in srs_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([srs_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([srs_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if srs_c[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += srs_c[index]
                        model_molecule[n_obj].positions[-1] = srs_c.positions[index] + np.dot(offset, srs_c.get_cell())
    ...
    for atom in srs_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, srs_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
