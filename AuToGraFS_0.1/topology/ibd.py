...
def make_ibd(names,sizes):
    ...
    ibd = crystal(['C', 'N'], [(0.375, 0.0, 0.25), (0.25,0.25,0.25)], spacegroup=230, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(ibd),len(ibd)) )
    pbc_bond_matrix = np.zeros( (len(ibd),len(ibd)) )
    ...
    for atom in ibd:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, ibd[index].symbol, offset, ibd.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in ibd:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([ibd[index].symbol for index in indices])
            symbol_string = ''.join(sorted([ibd[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += ibd[index]
                    model_molecule[n_obj].positions[-1] = ibd.positions[index] + np.dot(offset, ibd.get_cell())
    ...
    for atom in ibd:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, ibd[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
