...
def make_pyr(names,sizes):
    ...
    pyr = crystal(['C', 'N'], [(0.33333, 0.33333, 0.33333), (0.0,0.0,0.0)], spacegroup=205, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(pyr),len(pyr)) )
    pbc_bond_matrix = np.zeros( (len(pyr),len(pyr)) )
    ...
    for atom in pyr:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, pyr[index].symbol, offset, pyr.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in pyr:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([pyr[index].symbol for index in indices])
            symbol_string = ''.join(sorted([pyr[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += pyr[index]
                    model_molecule[n_obj].positions[-1] = pyr.positions[index] + np.dot(offset, pyr.get_cell())
    ...
    for atom in pyr:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, pyr[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
