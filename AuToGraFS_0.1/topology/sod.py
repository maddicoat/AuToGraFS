...
def make_sod(names,sizes):
    ...
    sod = crystal(['C', 'N' ], [(0.25, 0.0, 0.5), (0.125, 0.5, 0.125)], spacegroup=229, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(sod),len(sod)) )
    pbc_bond_matrix = np.zeros( (len(sod),len(sod)) )
    ...
    for atom in sod:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, sod[index].symbol, offset, sod.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in sod:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([sod[index].symbol for index in indices])
            symbol_string = ''.join(sorted([sod[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += sod[index]
                    model_molecule[n_obj].positions[-1] = sod.positions[index] + np.dot(offset, sod.get_cell())
    ...
    for atom in sod:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, sod[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
