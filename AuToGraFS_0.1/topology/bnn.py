...
def make_bnn(names,sizes):
    ...
    bnn = crystal(['C', 'N', 'N'], [(0.33333, 0.666667, 0.0), (0.5,0.5,0.0), (0.33333, 0.666667, 0.5)], spacegroup=191, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(bnn),len(bnn)) )
    pbc_bond_matrix = np.zeros( (len(bnn),len(bnn)) )
    ...
    for atom in bnn:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, bnn[index].symbol, offset, bnn.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in bnn:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([bnn[index].symbol for index in indices])
            symbol_string = ''.join(sorted([bnn[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += bnn[index]
                    model_molecule[n_obj].positions[-1] = bnn.positions[index] + np.dot(offset, bnn.get_cell())
    ...
    for atom in bnn:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, bnn[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
