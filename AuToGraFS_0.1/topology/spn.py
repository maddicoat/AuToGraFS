...
def make_spn(names,sizes):
    ...
    spn = crystal(['C', 'N'], [(0.16667, 0.16667, 0.16667), (0.0,0.0,0.0)], spacegroup=227, setting=2, 
    ...
    for atom in spn:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, spn[index].symbol, offset, spn.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in spn:
        if atom.symbol == 'N':
            ...
            symbol_string = ''.join(sorted([spn[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj].positions[-1] = spn.positions[index] + np.dot(offset, spn.get_cell())
