...
def make_eta_c(names,sizes):
    ...
    a = 3.1018 * factor 
    b = 2.1932 * factor #RCSR has 1.0932
    
    # C -> triangles, N => midpoints
    eta_c = crystal(['C', 'N' , 'N'], [(0.4069, 0.8138, 0.0), (0.5, 0.0, 0.0), (0.406900, 0.703450, 0.16666)], spacegroup=180, #P6(2)22 #3rd coord also wrong on RCSR
                   cellpar=[a, a, b, 90, 90, 120])
    
    
    #write('test.xyz',eta_c)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(eta_c),len(eta_c)) )
    pbc_bond_matrix = np.zeros( (len(eta_c),len(eta_c)) )
    ...
    for atom in eta_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, eta_c[index].symbol, offset, eta_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in eta_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([eta_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([eta_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if eta_c[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += eta_c[index]
                        model_molecule[n_obj].positions[-1] = eta_c.positions[index] + np.dot(offset, eta_c.get_cell())
    ...
    for atom in eta_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, eta_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
