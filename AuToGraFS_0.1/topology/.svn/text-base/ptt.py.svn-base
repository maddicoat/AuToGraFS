...
def make_ptt(names,sizes):
    ...
    ptt = crystal(['C', 'N', 'O'], [(0.14010, 0.0, 0.75), (0.0,0.5,0.75), (0.25,0.25,0.5)], spacegroup=66, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(ptt),len(ptt)) )
    pbc_bond_matrix = np.zeros( (len(ptt),len(ptt)) )
    ...
    for atom in ptt:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, ptt[index].symbol, offset, ptt.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in ptt:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([ptt[index].symbol for index in indices])
            symbol_string = ''.join(sorted([ptt[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += ptt[index]
                    model_molecule[n_obj].positions[-1] = ptt.positions[index] + np.dot(offset, ptt.get_cell())
    ...
    for atom in ptt:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, ptt[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
    ...
    ##Now we do the  O  (squares)
    for atom in ptt:
        if atom.symbol == 'O':
            #print'======================================='
            #print 'O Atom ',atom.index, " finding squares2"
            n_obj+=1
            model_molecule[n_obj] = Atoms()
            model_molecule[n_obj] += atom
            indices, offsets = nlist.get_neighbors(atom.index)
    ...
    for obj in xrange(n_centers+1,n_sq1+1):
    ...
    for obj in xrange(n_sq1+1,n_obj+1):
        f.write('\n%-8s %-3d\n' %('Linker: ', obj-n_sq1))
        f.write('%-3d\n' %(len(model_molecule[obj])))
        f.write('%-20s\n' %('type = rectangle'))
        #process positions to make it a bit more ideal
