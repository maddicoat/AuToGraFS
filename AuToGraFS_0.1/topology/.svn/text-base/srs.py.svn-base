...
def make_srs(names,sizes):
    ...
    srs = crystal(['C', 'N'], [(0.125, 0.125, 0.125), (0.5,0.25,0.875)], spacegroup=214,  
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(srs),len(srs)) )
    ...
    for atom in srs:
        if atom.symbol == 'C':
            ...
            symbols = ([srs[index].symbol for index in indices])
            symbol_string = ''.join(sorted([srs[index].symbol for index in indices]))
            #print symbol_string
            #for i,o in zip(indices, offsets):
            #    print i,o
            for index,offset in zip(indices,offsets):
                dist_mic = srs.get_distance(atom.index,index,mic=True)
                dist_no_mic = srs.get_distance(atom.index,index,mic=False)
                ...
                if (abs(dist_mic - dist_no_mic) <= eps) and (abs(dist_mic - factor/2.0) < eps): 
                     model_molecule[n_obj] += srs[index]
                    ...
                    print "Tag, ", srs[index].tag, " goes over pbc"
                    srs[index].tag = -(srs[index].tag)
                    model_molecule[n_obj] += srs[index]
                    model_molecule[n_obj].positions[-1] = srs.positions[index] + np.dot(offset, srs.get_cell())
                    model_molecule[n_obj][-1].tag = -bond_matrix[atom.index,index]
