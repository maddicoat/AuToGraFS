...
def make_pcu(names,sizes):
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(pcu),len(pcu)) )
    pbc_bond_matrix = np.zeros( (len(pcu),len(pcu)) )
    ...
    for atom in pcu:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, pcu[index].symbol, offset, pcu.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in pcu:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([pcu[index].symbol for index in indices])
            symbol_string = ''.join(sorted([pcu[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += pcu[index]
                    model_molecule[n_obj].positions[-1] = pcu.positions[index] + np.dot(offset, pcu.get_cell())
    ...
    for atom in pcu:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, pcu[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
