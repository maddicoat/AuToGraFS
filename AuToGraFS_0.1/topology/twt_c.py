...
def make_twt_c(names,sizes):
    ...
    #twt_c = crystal(['C', 'N' ,'N'], [(0.298, 0.0, 0.0), (0.149, 0.851, 0.333333), (0.5, 0.0, 0.0)], spacegroup=180,  #P6(2)22
    #Above line (RCSR coords) causes Very slight imprecision issue:
    #two extra atoms get created by the first N - they end up on the "top plane" of the cell and overlap with bottom plane equivs
    twt_c = crystal(['C', 'N' ,'N'], [(0.298, 0.0, 0.0), (0.149, 0.298, 0.0), (0.5, 0.0, 0.0)], spacegroup=180,  #P6(2)22
                   cellpar=[a, a, b, 90, 90, 120])
   
    #write('test.xyz',twt_c)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(twt_c),len(twt_c)) )
    pbc_bond_matrix = np.zeros( (len(twt_c),len(twt_c)) )
    ...
    for atom in twt_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, twt_c[index].symbol, offset, twt_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in twt_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([twt_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([twt_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if twt_c[index].symbol == 'N': # and abs(twt_c.get_distance(atom.index,index,mic=True) - factor) < eps:
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += twt_c[index]
                        model_molecule[n_obj].positions[-1] = twt_c.positions[index] + np.dot(offset, twt_c.get_cell())
    ...
    for atom in twt_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, twt_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
                    ...
                    #Then 
                if twt_c[index].symbol == 'C' :#and abs(twt_c.get_distance(atom.index,index,mic=True) - factor) < eps:
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += twt_c[index]
                        model_molecule[n_obj].positions[-1] = twt_c.positions[index] + np.dot(offset, twt_c.get_cell())
