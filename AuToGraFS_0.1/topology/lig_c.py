...
def make_lig_c(names,sizes):
    ...
    lig_c = crystal(['C', 'N' , 'N'], [(0.125, 0.125, 0.0), (0.0, 0.0, 0.0), (0.125, 0.25, 0.25)], 
            spacegroup=134, setting=2,#P4(2)/nnm :2
            cellpar=[a, a, b, 90, 90, 90])
    
    
    #write('test.xyz',lig_c)
    ...
    nlist = NeighborList(cov_rad,skin=0.15,self_interaction=False,bothways=True)
    nlist.build(lig_c)
    
    #To sort out tags, we need to label each bond
    nbond = 1
    bond_matrix = np.zeros( (len(lig_c),len(lig_c)) )
    pbc_bond_matrix = np.zeros( (len(lig_c),len(lig_c)) )
    ...
    for atom in lig_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, lig_c[index].symbol, offset, lig_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in lig_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([lig_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([lig_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if lig_c[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += lig_c[index]
                        model_molecule[n_obj].positions[-1] = lig_c.positions[index] + np.dot(offset, lig_c.get_cell())
    ...
    for atom in lig_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, lig_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
