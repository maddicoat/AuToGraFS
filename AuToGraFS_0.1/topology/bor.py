...
def make_bor(names,sizes):
    ...
    bor = crystal(['C', 'N'], [(0.16667, 0.16667, 0.16667), (0.5,0.0,0.0)], spacegroup=215, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(bor),len(bor)) )
    pbc_bond_matrix = np.zeros( (len(bor),len(bor)) )
    ...
    for atom in bor:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, bor[index].symbol, offset, bor.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in bor:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([bor[index].symbol for index in indices])
            symbol_string = ''.join(sorted([bor[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += bor[index]
                    model_molecule[n_obj].positions[-1] = bor.positions[index] + np.dot(offset, bor.get_cell())
    ...
    for atom in bor:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, bor[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
