...
def make_qom(names,sizes):
    ...
    # C -> octahedra, N => triangles
    qom = crystal(['C', 'C', 'N', 'N', 'N' ], [(0.33333, 0.66667, 0.75), (0.1667, 0.8333, 0.25), 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(qom),len(qom)) )
    pbc_bond_matrix = np.zeros( (len(qom),len(qom)) )
    ...
    for atom in qom:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, qom[index].symbol, offset, qom.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in qom:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([qom[index].symbol for index in indices])
            symbol_string = ''.join(sorted([qom[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if qom[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += qom[index]
                        model_molecule[n_obj].positions[-1] = qom.positions[index] + np.dot(offset, qom.get_cell())
    ...
    for atom in qom:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, qom[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
