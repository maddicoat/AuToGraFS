...
def make_ths_c(names,sizes):
    ...
    ths_c = crystal(['C', 'N' , 'N'], [(0.75, 0.25, 0.9375), (0.25, 0.75, 0.25), (0.0, 0.0, 0.0)], spacegroup=134, setting=2, #P4(2)/nnm
                   cellpar=[a, a, b, 90, 90, 90])
    
    
    #write('test.xyz',ths_c)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(ths_c),len(ths_c)) )
    pbc_bond_matrix = np.zeros( (len(ths_c),len(ths_c)) )
    ...
    for atom in ths_c:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, ths_c[index].symbol, offset, ths_c.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in ths_c:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([ths_c[index].symbol for index in indices])
            symbol_string = ''.join(sorted([ths_c[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if ths_c[index].symbol == 'N':
                    if any(o != 0 for o in offset):
                        #If we're going over a periodic boundary, we need to negate the tag
                        model_molecule[n_obj] += ths_c[index]
                        model_molecule[n_obj].positions[-1] = ths_c.positions[index] + np.dot(offset, ths_c.get_cell())
    ...
    for atom in ths_c:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, ths_c[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
