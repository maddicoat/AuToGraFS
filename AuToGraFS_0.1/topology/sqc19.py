...
def make_sqc19(names,sizes):
    ...
    # C -> 12-hedra, N => edges 
    sqc19 = crystal(['C', 'N'], [(0.0, 0.0, 0.0), (-0.25, -0.25, 0.0)], spacegroup=225,  #Fm-3m
                   cellpar=[a, a, a, 90, 90, 90])
    
    
    write('test.xyz',sqc19)
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(sqc19),len(sqc19)) )
    pbc_bond_matrix = np.zeros( (len(sqc19),len(sqc19)) )
    ...
    for atom in sqc19:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, sqc19[index].symbol, offset, sqc19.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in sqc19:
        if atom.symbol == 'C':
            ...
            print indices
            print offsets
            symbols = ([sqc19[index].symbol for index in indices])
            symbol_string = ''.join(sorted([sqc19[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += sqc19[index]
                    model_molecule[n_obj].positions[-1] = sqc19.positions[index] + np.dot(offset, sqc19.get_cell())
    ...
    for atom in sqc19:
        if atom.symbol == 'N':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, sqc19[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
