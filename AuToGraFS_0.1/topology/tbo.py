...
def make_tbo(names,sizes):
    ...
    # C -> triangles, N => squares
    tbo = crystal(['C', 'N'], [(0.33333, 0.33333, 0.33333), (0.25,0.0,0.25)], spacegroup=225, 
    ...
    nbond = 1
    bond_matrix = np.zeros( (len(tbo),len(tbo)) )
    pbc_bond_matrix = np.zeros( (len(tbo),len(tbo)) )
    ...
    for atom in tbo:
        ...
        for index,offset in zip(indices,offsets):
            print atom.index, index, atom.symbol, tbo[index].symbol, offset, tbo.get_distance(atom.index,index,mic=True)
            if atom.index < index:
    ...
    for atom in tbo:
        if atom.symbol == 'N':
            ...
            print indices
            print offsets
            symbols = ([tbo[index].symbol for index in indices])
            symbol_string = ''.join(sorted([tbo[index].symbol for index in indices]))
            #print symbol_string
            ...
            for index,offset in zip(indices,offsets):
                ...
                if any(o != 0 for o in offset):
                    ...
                    model_molecule[n_obj] += tbo[index]
                    model_molecule[n_obj].positions[-1] = tbo.positions[index] + np.dot(offset, tbo.get_cell())
    ...
    for atom in tbo:
        if atom.symbol == 'C':
            ...
            indices, offsets = nlist.get_neighbors(atom.index)
            for index,offset in zip(indices,offsets):
                print index, offset, tbo[index].symbol
                this_bond = [atom.index, index]
                for o in offset:
                    this_bond.append(o)
                bond = tuple(this_bond)
    ...
    for obj in model_molecule:
        test_mol += model_molecule[obj]
        test_mol.set_cell(tbo.get_cell())
        test_mol.set_pbc(tbo.get_pbc())
